...
import { format, isSameDay, startOfDay, endOfDay, differenceInCalendarDays, getDay, getHours } from "date-fns";
...
export default function Home() {
  ...
  const platformStats = filteredOrders.reduce((acc, o) => {
    ...
    acc[p].orders += 1;
    acc[p].revenue += (o.total_amount || 0);
    return acc;
  }, {} as Record<string, { orders: number, revenue: number }>);

  const menuStats = filteredOrders.reduce((acc, o) => {
    (o.items || []).forEach((item) => {
      const name = item.dish_name || "Unknown Dish";
  ...
  filteredOrders.forEach(o => {
    ...
    if (hour === 0 && o.last_updated?._seconds) {
      d = new Date(o.last_updated._seconds * 1000);
      hour = getHours(d);
    }
    
    if (hour === 0 && o.order_id?.includes('-L')) hour = 11;
    if (hour === 0 && o.order_id?.includes('-D')) hour = 18;
    ...
    if (slotIdx !== -1) {
      if (heatmapMode === 'orders') {
        ...
        const itemQty = (o.items || []).reduce((sum, item) => sum + (item.quantity || 0), 0);
